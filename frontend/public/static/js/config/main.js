...
list.register(new Observer('append', async item => {
    ...
    try {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess('Configuración creada exitosamente!');
            renderApp();
            return;
        }
        ...
        showToastError('¡No se puede crear la configuración!');
        list.remove(item.id);
        categories.remove(item.id);
    }
...
list.register(new Observer('update', async item => {
    ...
    try {
        ...
        if (response.status === 200) {
            showToastSuccess('Configuración actualizada exitosamente!');
            return;
        }
        ...
        showToastError('No se puede actualizar la configuración!');
    } finally {
        renderApp();
    }
}));
...
list.register(new Observer('remove', async id => {
    ...
    try {
        ...
        showToastError('No se puede eliminar la configuración!');
    } finally {
        renderApp();
    }
}));
