...
const main = async () => {
    ...
    categoryList.register(new Observer('add', async category => {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess(`Categoria ${category.name} creada exitosamente!`)
            render()
            return
        }
    ...
    categoryList.register(new Observer('remove', async category => {
        ...
        try {
            ...
            if (response.status == 204) {
                showToastSuccess(`Categoria ${category.name} removida exitosamente!`)
                return
            }
            ...
            showToastError(`Error al eliminar la categoría ${category.name}!`)
        }
    ...
    categoryList.register(new Observer('update', async category => {
        ...
        if (data.status == 200) {
            showToastSuccess(`Categoria ${category.name} actualizada exitosamente!`)
            return;
        }

        showToastError(`Error al actualizar la categoría ${category.name}!`)
    }))
